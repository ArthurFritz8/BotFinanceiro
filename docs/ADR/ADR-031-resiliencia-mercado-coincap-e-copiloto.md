...
102. Plataforma recebeu modulo dedicado de ETFs com endpoints `GET /v1/etfs/snapshot`, `GET /v1/etfs/snapshot/batch` e `GET /v1/etfs/market-overview`, cobrindo cestas broad market, tematicas, internacionais e renda fixa.
103. Plataforma recebeu modulo dedicado de setores globais com endpoints `GET /v1/global-sectors/snapshot`, `GET /v1/global-sectors/snapshot/batch` e `GET /v1/global-sectors/market-overview`, incluindo leitura de forca relativa e breadth (advance/decline).
104. Plataforma recebeu modulo dedicado de macro rates com endpoints `GET /v1/macro-rates/snapshot`, `GET /v1/macro-rates/snapshot/batch` e `GET /v1/macro-rates/market-overview`, consolidando curva de juros, dolar, VIX e proxies de regime de risco.
105. Copiloto IA foi expandido com novas tools read-only (`get_etfs_market_snapshot`, `get_global_sectors_market_snapshot` e `get_macro_rates_market_snapshot`), ampliando analise interclasse para alocacao, rotacao setorial e contexto macro no mesmo fluxo conversacional.
106. Frontend ganhou novos atalhos inteligentes para ETFs, Setores Globais e Macro Rates, reduzindo friccao operacional para consultas taticas e macro sem alterar o fluxo principal do chat.
107. Plataforma recebeu modulo dedicado de Portfolios com endpoints `GET /v1/portfolios/snapshot`, `GET /v1/portfolios/snapshot/batch` e `GET /v1/portfolios/market-overview`, incluindo presets de carteira, pesos customizados, exposicao por classe e score de risco ponderado.
108. Copiloto IA foi expandido com nova tool read-only `get_portfolio_risk_snapshot`, habilitando analise de diversificacao, risco e regime de carteira no mesmo fluxo conversacional multi-mercado.
109. Frontend ganhou novo atalho inteligente para diagnostico de carteira (pesos customizados), reduzindo tempo operacional para avaliacao de risco/alocacao sem alterar o fluxo principal do chat.
