...
void it("GET /v1/crypto/chart retorna pontos e insights tecnicos", async () => {
    ...
    data: {
      ...
      insights: {
        ...
        marketStructure: {
          ...
          fairValueGapBias: "bearish" | "bullish" | "none";
          ...
          liquiditySweepSignal: "bearish" | "bullish" | "none";
  ...
  assert.equal(typeof body.data.insights.marketStructure.fairValueGapActive, "boolean");
  assert.ok(["none", "bullish", "bearish"].includes(body.data.insights.marketStructure.fairValueGapBias));
  assert.ok(body.data.insights.marketStructure.fairValueGapLower === null || typeof body.data.insights.marketStructure.fairValueGapLower === "number");
  assert.ok(body.data.insights.marketStructure.fairValueGapUpper === null || typeof body.data.insights.marketStructure.fairValueGapUpper === "number");
  assert.ok(["discount", "equilibrium", "premium"].includes(body.data.insights.marketStructure.institutionalZone));
  assert.ok(["none", "bullish", "bearish"].includes(body.data.insights.marketStructure.liquiditySweepSignal));
  assert.ok(body.data.insights.marketStructure.liquiditySweepReferenceHigh === null || typeof body.data.insights.marketStructure.liquiditySweepReferenceHigh === "number");
  assert.ok(body.data.insights.marketStructure.liquiditySweepReferenceLow === null || typeof body.data.insights.marketStructure.liquiditySweepReferenceLow === "number");
