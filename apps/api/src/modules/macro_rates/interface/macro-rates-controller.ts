...
import { MacroRatesMarketService } from "../application/macro-rates-market-service.js";
...
export async function getMacroRatesSnapshot(
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<void> {
  const parsedQuery = snapshotQuerySchema.parse(request.query);
  const snapshot = await macroRatesMarketService.getSnapshot({
...
export async function getMacroRatesSnapshotBatch(
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<void> {
  const parsedQuery = snapshotBatchQuerySchema.parse(request.query);
  const batch = await macroRatesMarketService.getSnapshotBatch({
...
export async function getMacroRatesMarketOverview(
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<void> {
  const parsedQuery = marketOverviewQuerySchema.parse(request.query);
  const overview = await macroRatesMarketService.getMarketOverview({
