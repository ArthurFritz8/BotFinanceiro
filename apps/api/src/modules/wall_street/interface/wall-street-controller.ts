...
import { WallStreetMarketService } from "../application/wall-street-market-service.js";
...
export async function getWallStreetSnapshot(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const parsedQuery = snapshotQuerySchema.parse(request.query);
  const snapshot = await wallStreetMarketService.getSnapshot({
...
export async function getWallStreetSnapshotBatch(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const parsedQuery = snapshotBatchQuerySchema.parse(request.query);
  const batch = await wallStreetMarketService.getSnapshotBatch({
...
export async function getWallStreetMarketOverview(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const parsedQuery = marketOverviewQuerySchema.parse(request.query);
  const overview = await wallStreetMarketService.getMarketOverview({
