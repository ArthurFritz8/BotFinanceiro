...
import { CommoditiesMarketService } from "../application/commodities-market-service.js";
...
const snapshotQuerySchema = z.object({
  symbol: symbolSchema.default("GC=F"),
});
...
export async function getCommoditiesSnapshot(
  ...
  const snapshot = await commoditiesMarketService.getSnapshot({
...
export async function getCommoditiesSnapshotBatch(
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<void> {
  const parsedQuery = snapshotBatchQuerySchema.parse(request.query);
  const batch = await commoditiesMarketService.getSnapshotBatch({
...
export async function getCommoditiesMarketOverview(
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<void> {
  const parsedQuery = marketOverviewQuerySchema.parse(request.query);
  const overview = await commoditiesMarketService.getMarketOverview({
