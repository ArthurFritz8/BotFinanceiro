...
void it("GET /internal/copilot/audit/history aplica paginacao com filtros combinados mantendo ordenacao", async () => {
  copilotStoreInternal.records = [
    {
      completion: {
    ...
    },
    {
      completion: {
        answer: "Outro tool",
        fetchedAt: "2026-04-01T09:15:01.000Z",
        model: "google/gemini-1.5-flash",
        provider: "openrouter",
        responseId: "copilot-audit-page-004",
        toolCallsUsed: ["get_crypto_sync_policy"],
        usage: {
          totalTokens: 33,
        },
  ...
  const response = await app.inject({
    ...
    method: "GET",
    url: `/internal/copilot/audit/history?limit=1&offset=1&sessionId=sessao_copilot_pagination&toolName=get_crypto_multi_spot_price&from=${encodeURIComponent(fromTimestamp)}&to=${encodeURIComponent(toTimestamp)}`,
  });
  ...
  assert.equal(body.data.totalStored, 4);
  assert.equal(body.data.totalMatched, 3);
  assert.equal(body.data.records.length, 1);
  assert.equal(body.data.records[0]?.sessionId, "sessao_copilot_pagination");
  assert.equal(body.data.records[0]?.completion.responseId, "copilot-audit-page-002");
...
void it("GET /internal/copilot/audit/history retorna 400 para offset fora do intervalo", async () => {
  const response = await app.inject({
    headers: {
      "x-internal-token": process.env.INTERNAL_API_TOKEN ?? "",
    },
    method: "GET",
    url: "/internal/copilot/audit/history?offset=-1",
  });
