...
import { EquitiesMarketService } from "../application/equities-market-service.js";
...
const snapshotQuerySchema = z.object({
  symbol: symbolSchema.default("AAPL"),
});
...
export async function getEquitiesSnapshot(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const parsedQuery = snapshotQuerySchema.parse(request.query);
  const snapshot = await equitiesMarketService.getSnapshot({
...
export async function getEquitiesSnapshotBatch(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const parsedQuery = snapshotBatchQuerySchema.parse(request.query);
  const batch = await equitiesMarketService.getSnapshotBatch({
...
export async function getEquitiesMarketOverview(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const parsedQuery = marketOverviewQuerySchema.parse(request.query);
  const overview = await equitiesMarketService.getMarketOverview({
