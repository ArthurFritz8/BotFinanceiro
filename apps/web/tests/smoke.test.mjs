...
test("main.js aplica AUTO inteligente com estabilidade no chart", async () => {
  ...
  assert.match(mainSource, /buildLiquidityHeatmapSnapshot/);
  assert.match(mainSource, /function renderSmcLiquidityHeatmapPanel\(view, currency\)/);
  assert.match(mainSource, /liquidityHeatmap\.zones/);
  assert.match(mainSource, /id="smc-liquidity-heatmap-panel"/);
  ...
  assert.match(mainSource, /buildExecutionPlanSnapshot/);
  assert.match(mainSource, /function renderTimingExecutionPlanPanel\(executionPlan, currency\)/);
  assert.match(mainSource, /id="timing-execution-plan-panel"/);
  assert.match(mainSource, /plan\.risk/);
  assert.match(mainSource, /createExecutionJournalEntry/);
  assert.match(mainSource, /function renderTimingExecutionJournalPanel\(journal, recentEntries, currency\)/);
  assert.match(mainSource, /id="timing-execution-journal-panel"/);
  assert.match(mainSource, /data-execution-journal-action="record"/);
  assert.match(mainSource, /data-execution-journal-action="paper"/);
  assert.match(mainSource, /function updateExecutionChartVisualState\(executionGate, executionPlan, journal\)/);
